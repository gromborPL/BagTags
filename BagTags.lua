...
function BT.InventoryModule:RefreshAllOverlays()
  ...
  -- Po odświeżeniu nakładek, odśwież też przycisk Sell Vendor (jeśli dostępny)
  if BT and BT.UpdateSellButton then
    -- drobne opóźnienie pozwala GetItemInfo i UI uaktualnić się
    C_Timer.After(0.03, BT.UpdateSellButton)
  end
...
-- ===== Sell Vendor button & helpers (WSTAW PO FUNKCJI BT:FormatMoney) =====

local NUM_BAG_SLOTS_MAX = 4 -- 0..4 zwykłe baki (dostosuj jeśli chcesz obsłużyć bank itp.)
...
local function GetTagForSlot(bag, slot)
  -- Preferuj BT:GetValueTag (zdefiniowane w tym pliku), potem BT:GetItemTag / BT:GetItemTag-like
  ...
  if BT and type(BT.GetItemTag) == "function" then
    local ok, tag = pcall(BT.GetItemTag, BT, bag, slot)
    if ok and tag then return tag end
  end
  -- fallback: jeśli masz SafeGetContainerItemLink i BagTags-like storage, dodaj tutaj inne sposoby
  return nil
end

local function GetStackCountAt(bag, slot)
  -- używamy istniejącego helpera BT:GetContainerItemInfo / SafeGetContainerItemInfo
  if BT and type(BT.GetContainerItemInfo) == "function" then
    local ok, texture, count = pcall(BT.GetContainerItemInfo, BT, bag, slot)
    if ok then return count or 1 end
  end
  -- fallback na globalne API (stare)
  local _, count = GetContainerItemInfo(bag, slot)
  return count or 1
end

local function ComputeVendorTotal()
  local total = 0
  for bag = 0, NUM_BAG_SLOTS_MAX do
    local slots = (BT and BT.GetContainerNumSlots and BT:GetContainerNumSlots(bag)) or GetContainerNumSlots(bag) or 0
    if slots and slots > 0 then
      for slot = 1, slots do
        local link = (BT and BT.SafeGetContainerItemLink and BT:SafeGetContainerItemLink(bag, slot)) or GetContainerItemLink(bag, slot)
        if link then
          local tag = GetTagForSlot(bag, slot)
          if IsVendorTag(tag) then
            local itemName, _, _, _, _, _, _, _, _, _, vendorPrice = GetItemInfo(link)
            local stack = GetStackCountAt(bag, slot) or 1
...
-- UI: stwórz przycisk (parent dostosuj niżej)
-- wybierz najlepszy parent: najpierw ramka inventory modułu (jeśli istnieje),
-- potem globalna ramka BagTags (jeśli jest), potem fallback ContainerFrame1, potem UIParent
local parentForButton =
  (BT and BT.InventoryModule and BT.InventoryModule.frame)
  or (BT and BT.Frame)
  or _G["ContainerFrame1"]
  or UIParent -- możesz zmienić na konkretną ramkę BagTags, jeśli chcesz
local sellBtn = CreateFrame("Button", "BagTags_SellVendorBtn", parentForButton, "UIPanelButtonTemplate")
sellBtn:SetSize(160, 22)
sellBtn:SetPoint("TOPRIGHT", parentForButton, "TOPRIGHT", -10, -40) -- dostosuj anchor do swojego UI
sellBtn:SetText("Sell Vendor")
sellBtn:Hide()

-- dopasuj poziomy i parent (gdyby parent został ustawiony jako wynik powyżej)
sellBtn:SetParent(parentForButton)
if parentForButton and type(parentForButton.GetFrameLevel) == "function" then
  sellBtn:SetFrameLevel(parentForButton:GetFrameLevel() + 5)
end
if parentForButton and type(parentForButton.GetFrameStrata) == "function" then
  sellBtn:SetFrameStrata(parentForButton:GetFrameStrata())
end
-- Ustaw punkt względem parentForButton
sellBtn:ClearAllPoints()
sellBtn:SetPoint("TOPRIGHT", parentForButton, "TOPRIGHT", -8, -28)

-- podpinamy OnShow/OnHide jeśli parent jest ramką BagTags
if parentForButton and type(parentForButton.IsObjectType) == "function" and parentForButton:IsObjectType("Frame") then
  parentForButton:HookScript("OnShow", function() C_Timer.After(0.02, UpdateSellButton) end)
  parentForButton:HookScript("OnHide", function() sellBtn:Hide() end)
end

-- Kolejka sprzedaży (bezpieczne UseContainerItem z przerwami)
local sellQueue = {}
local sellFrame = CreateFrame("Frame")
local sellInterval = 0.12
local sellTimer = 0
...
local function StartSellingQueue()
  ...
  sellFrame:SetScript("OnUpdate", function(self, elapsed)
    ...
    if sellTimer >= sellInterval then
      ...
      -- upewnij się, że item nadal istnieje w danym slocie
      local currentLink = (BT and BT.SafeGetContainerItemLink and BT:SafeGetContainerItemLink(entry.bag, entry.slot)) or GetContainerItemLink(entry.bag, entry.slot)
      if currentLink then
...
-- Widoczność/tekst przycisku
local function UpdateSellButton()
  local total = ComputeVendorTotal()
  if total > 0 then
    sellBtn:SetText(("Sell Vendor — %s"):format(BT:FormatMoney(total)))
    sellBtn:Show()
...
-- Eventy: odświeżajemy gdy torby/merchant/enter world
local sellEvt = CreateFrame("Frame")
sellEvt:RegisterEvent("BAG_UPDATE")
sellEvt:RegisterEvent("BAG_UPDATE_DELAYED")
sellEvt:RegisterEvent("PLAYER_ENTERING_WORLD")
sellEvt:RegisterEvent("MERCHANT_SHOW")
sellEvt:RegisterEvent("MERCHANT_CLOSED")
-- jeżeli BT emituje własny event przy zmianie tagów, dodaj go tutaj (np. "BAGTAGS_UPDATED")

sellEvt:SetScript("OnEvent", function(self, event, ...)
  if event == "MERCHANT_SHOW" then
    UpdateSellButton()
  elseif event == "MERCHANT_CLOSED" then
    -- przerwij ewentualną sprzedaż i opróżnij kolejkę
    sellQueue = {}
    sellFrame:SetScript("OnUpdate", nil)
    UpdateSellButton()
  else
    -- małe odroczenie, żeby GetItemInfo zdążył (jeśli potrzebne)
    C_Timer.After(0.06, UpdateSellButton)
  end
end)

-- opcjonalny slash do szybkiego testu: /btsellvendor
SLASH_BAGTAGSVENDOR1 = "/btsellvendor"
SlashCmdList["BAGTAGSVENDOR"] = function()
  UpdateSellButton()
  if sellBtn:IsShown() then
    print(("BagTags: sell vendor total = %s"):format(BT:FormatMoney(ComputeVendorTotal())))
  else
    print("BagTags: no vendor-tagged items found.")
  end
end


-- expose dla InventoryModule / innych modułów
BT.UpdateSellButton = UpdateSellButton
